...
Starting with version 2.068, the front-end became self-hosting. With that version, ebuilds in this overlay started to offer compiler USE flags to pull in and use a specific host compiler. They are named after the compiler executable and version, e.g.: `ldc2-0_17`, `dmd-2_074`, `gdc-4_9_4`.
...
### GDC
GDC extends the GNU Compiler Collection (GCC) and benefits from its optimizer. The Dlang front-end version employed by GDC lags behind DMD's, such that recent language features may not be available. In this overlay you find patched `sys-devel/gcc` ebuilds that integrate GDC into the system's GCC.
...
To install GDC, enable the `d` USE-flag on `sys-devel/gcc` and then emerge one of the versions from this overlay via `emerge gcc::dlang`. If you are interested in a particular Dlang front-end version, refer to [dlang-compilers.eclass](https://github.com/gentoo/dlang/blob/master/eclass/dlang-compilers.eclass) to find out which GCC slot is associated with which front-end.
